 ...
 */
export function getBlackTexture(): Texture {
  if (!cachedTexture) {
    cachedTexture = new Texture(128, 128);
    applyBlackTexture(cachedTexture);
  }
 ...
 */
export function applyBlackTexture(texture: Texture) {
  // Clear/fill the texture with the dominant color
  texture.fillWithColor(new Color(0, 0, 0), 1);
